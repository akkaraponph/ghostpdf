...
func TestComparePDFFiles(t *testing.T) {
	...
	for i := 1; i <= 2; i++ {
		...
		filename := "test_compare_" + string(rune('0'+i)) + ".pdf"
	...
	// Compare the files
	err := gofpdf.ComparePDFFiles("test_compare_1.pdf", "test_compare_2.pdf", false)
	if err != nil {
...
func TestComparePDFFilesMissing(t *testing.T) {
	...
	err := pdf.OutputFileAndClose("test_compare_exists.pdf")
	if err != nil {
		t.Fatalf("Failed to create test PDF: %v", err)
	}

	// Compare with non-existent file (should succeed as per implementation)
	err = gofpdf.ComparePDFFiles("test_compare_exists.pdf", "nonexistent.pdf", false)
	if err != nil {
